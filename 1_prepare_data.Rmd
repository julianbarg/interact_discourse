...
data <- data %>%
  map(~ modify_at(.x, "preprocessing",
                  ~ fix_collocations(.x, compounds_5$collocation)))
file.remove(here("output", "compounds.txt"))
file.create(here("output", "compounds.txt"))
cat(compounds_5$collocation, file = here("output", "compounds.txt"),
    sep = "\n", append = T)
```
