...
```{r steering_plot_3, warning=FALSE}
steering_plot_3 <- steering_sample_3 %>%
  # filter(group %in% c("anti", "science")) %>%
  # filter(group == "anti") %>%
  # filter(n <= 20) %>%
  # group_by(title, id) %>%
  ...
  ggplot(aes(n, dist)) +
    ...
    geom_jitter(aes(n, dist, fill = description), 
                size = 0.2, height = 0.015) +
    labs(title = "Steering of witness testimony by initial topic", 
         x = "Witness responses to examiner question #", 
         y = "Degree of topic change from original testimony", 
         caption = paste0("Topic change measured as euclidean distance\n", 
                          "Smoothing: squareroot function")) +
    theme_bw() +
    theme(
...
ggsave(here("output", "plot_witness_steering_3.png"), steering_plot_3, 
       width = 6.5, height = 8.5
       # width = 10, height = 6.5
       )
knitr::include_graphics(here("output", "plot_witness_steering_3.png"))
```
